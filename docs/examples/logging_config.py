    ...
    if enable_file and os.path.exists(log_dir):
        try:
            log_file = os.path.join(
                log_dir, f"fetcher-{datetime.now(UTC).strftime('%Y%m%d')}.log"
            )
        ...
        try:
            ...
            try:
                ...
                if ready.status_code == 200:
                    ...
                    batch = (
                        batch_env == "true" or batch_env == "1" or batch_env == "yes"
                    )
                else:
                    logger.warning(
                        f"Loki not ready (status {ready.status_code}), using batch handler"
                    )
            except Exception:
            ...
            loki_formatter = CustomJsonFormatter(
