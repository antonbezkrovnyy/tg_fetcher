...
class FetcherConfig:
    ...
    def _validate_ranges(self):
        ...
        if not (0.1 <= self.retry_delay <= 300.0):
            raise ConfigValidationError(
                "retry_delay must be between 0.1 and 300 seconds"
            )
    ...
    def _sanitize_chats(self):
        ...
        if not self.chats:
            raise ConfigValidationError(
                "at least one valid chat must be specified after sanitization"
            )
    ...
    @classmethod
    def from_file_with_env_override(
...
def create_example_config(output_path: Union[str, Path]) -> None:
    ...
    example_config = {
        ...
        "rate_limit": {"calls_per_second": 5.0, "burst_size": 3, "window_size": 60},
    }
