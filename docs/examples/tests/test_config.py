...
class TestConfigManager:
    ...
    def test_config_from_file(self, temp_data_dir):
        ...
        config_data = {
            ...
            "rate_limit": {"calls_per_second": 5, "burst_size": 3},
        }
    ...
    def test_config_validation_required_fields(self):
        ...
        with pytest.raises(ConfigValidationError, match="api_hash is required"):
            FetcherConfig(api_id=12345, api_hash="", chats=["channel1"])

        # Empty chats list should raise validation error
        with pytest.raises(
            ConfigValidationError, match="at least one chat must be specified"
        ):
            FetcherConfig(api_id=12345, api_hash="test", chats=[])
    ...
    def test_config_validation_ranges(self):
        ...
        # Invalid max_retries
        with pytest.raises(
            ConfigValidationError, match="max_retries must be between 1 and 10"
        ):
            FetcherConfig(
                api_id=12345, api_hash="test", chats=["channel1"], max_retries=0
            )

        # Invalid concurrent_channels
        with pytest.raises(
            ConfigValidationError, match="concurrent_channels must be between 1 and 20"
        ):
            FetcherConfig(
                api_id=12345,
                api_hash="test",
    ...
    def test_config_defaults(self):
        ...
        config = FetcherConfig(api_id=12345, api_hash="test", chats=["channel1"])
    ...
    def test_config_env_override_file(self, temp_data_dir):
        ...
        env_vars = {"API_ID": "22222", "MAX_RETRIES": "7"}
    ...
    def test_rate_limit_config(self):
        ...
        rate_config = RateLimitConfig(calls_per_second=5, burst_size=3, window_size=60)
...
class TestConfigConstants:
    ...
    def test_default_paths(self):
        ...
        from config import DEFAULT_DATA_DIR, DEFAULT_PROGRESS_FILE, DEFAULT_SESSION_DIR
