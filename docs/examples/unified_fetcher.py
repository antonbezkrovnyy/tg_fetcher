...
def validate_arguments(args):
    ...
    if args.mode == 'range':
        if not args.date_from or not args.date_to:
            sys.stderr.write("Error: --from and --to are required when using --mode range\n")
            return False
            
    # Validate date formats
    for date_str, name in [(args.date, 'date'), (args.date_from, 'from'), (args.date_to, 'to')]:
        if date_str:
            try:
...
async def main():
    ...
    try:
        ...
        if args.mode == 'yesterday':
            ...
            config.fetch_mode = FetchMode.CUSTOM_DATE.value if hasattr(FetchMode, 'CUSTOM_DATE') else 'date'
            config.target_date = datetime.strptime(args.date, '%Y-%m-%d').date()
            
        elif args.mode == 'range':
            # Special date range mode  
            config.fetch_mode = FetchMode.DATE_RANGE.value if hasattr(FetchMode, 'DATE_RANGE') else 'range'
