...
class Reaction(BaseModel):
    ...
    users: Optional[list[int]] = Field(
        default=None,
        description="User IDs who reacted (if available)"
    )
...
class Message(BaseModel):
    ...
    comments: list["Message"] = Field(
        default_factory=list,
        description="Comments/replies in discussion thread"
    )
...
class MessageCollection(BaseModel):
    ...
    senders: dict[str, str] = Field(
        ...
        description="Mapping of sender_id to display_name"
    )
...
class ProgressEntry(BaseModel):
    ...
    last_message_id: Optional[int] = Field(
        default=None,
        description="ID of last processed message"
    )
    ...
    completed_dates: list[str] = Field(
        default_factory=list,
        description="List of fully processed dates (YYYY-MM-DD)"
    )
...
class ProgressFile(BaseModel):
    ...
    sources: dict[str, ProgressEntry] = Field(
        default_factory=dict,
        description="Progress entries keyed by source ID"
    )
