...
class FetcherConfig(BaseSettings):
    ...
    fetch_date: Optional[date] = Field(
        default=None,
        description="Specific date for fetch_mode=date (YYYY-MM-DD)"
    )
    fetch_start: Optional[date] = Field(
        default=None,
        description="Start date for fetch_mode=range (YYYY-MM-DD)"
    )
    fetch_end: Optional[date] = Field(
        default=None,
        description="End date for fetch_mode=range (YYYY-MM-DD)"
    )
    
    # === Paths ===
    data_dir: Path = Field(
        default=Path("./data"),
        description="Directory for storing fetched messages"
    )
    session_dir: Path = Field(
        default=Path("./sessions"),
        description="Directory for Telegram session files"
    )
    ...
    max_parallel_channels: int = Field(
        ...
        ge=1,
        le=10,
        description="Maximum channels to process in parallel"
    )
    ...
    progress_reset: bool = Field(
        default=False,
        description="Reset all progress and start from scratch"
    )
    ...
    loki_url: Optional[str] = Field(
        default=None,
        description="Loki URL for log shipping (e.g., http://loki:3100)"
    )
    ...
    def validate_mode_requirements(self) -> None:
        ...
        if self.fetch_mode == "range":
            if self.fetch_start is None or self.fetch_end is None:
                raise ValueError("fetch_start and fetch_end are required when fetch_mode=range")
